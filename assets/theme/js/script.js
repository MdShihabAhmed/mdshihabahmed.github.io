 ...
 * @return {undefined}
 */
$jscomp.polyfill = function(n, tag, obj, name) {
  if (tag) {
    obj = $jscomp.global;
    n = n.split(".");
    /** @type {number} */
    name = 0;
    for (; name < n.length - 1; name++) {
      var prop = n[name];
      if (!(prop in obj)) {
        obj[prop] = {};
      }
...
$jscomp.polyfill("Array.prototype.includes", function(isDefault) {
  return isDefault ? isDefault : function(b, k) {
    var target = this;
  ...
  DOMLoaded(function() {
    ...
    function init(a) {
      f(a, "[data-bg-video]").forEach(function(a) {
        var entry = a.getAttribute("data-bg-video");
        if (entry) {
          var meta = entry.match(/(http:\/\/|https:\/\/|)?(player.|www.)?(vimeo\.com|youtu(be\.com|\.be|be\.googleapis\.com))\/(video\/|embed\/|watch\?v=|v\/)?([A-Za-z0-9._%-]*)(&\S+)?/);
          ...
          if (meta && (/youtu\.?be/g.test(meta[3]) || /vimeo/g.test(meta[3]))) {
            if (meta && /youtu\.?be/g.test(meta[3])) {
              /** @type {string} */
              entry = "http" + ("https:" === location.protocol ? "s" : "") + ":";
              /** @type {string} */
              entry = entry + ("//img.youtube.com/vi/" + meta[6] + "/maxresdefault.jpg");
              /** @type {!Image} */
              var image = new Image;
              /**
               * @return {undefined}
              ...
              image.onload = function() {
                if (120 === (image.naturalWidth || image.width)) {
                  /** @type {string} */
                  var element = image.src.split("/").pop();
                  switch(element) {
                    case "maxresdefault.jpg":
                      /** @type {string} */
                      image.src = image.src.replace(element, "sddefault.jpg");
                      break;
                    case "sddefault.jpg":
                      /** @type {string} */
                      image.src = image.src.replace(element, "hqdefault.jpg");
                      break;
                    default:
                      if (err) {
                        /** @type {string} */
                ...
                var h = element.parentNode.clientHeight;
                ...
                self.on("playing", function() {
                  self.replayFrom(1);
                  if (self._ready && 0 <= self.getCurrentTime()) {
                    /** @type {string} */
              ...
              if (meta && /vimeo/g.test(meta[3])) {
                ...
                entry = new Vimeo.Player(element, {
                  id : entry,
                  loop : true,
                  background : true,
                  responsive : true,
                ...
                element = entry.element.parentNode;
                /** @type {string} */
                element.style.overflow = "hidden";
                /** @type {string} */
                entry.element.style.pointerEvents = "none";
                /** @type {string} */
                entry.element.style.marginLeft = "-" + (entry.element.scrollWidth - element.scrollWidth) / 2 + "px";
                /** @type {string} */
                entry.element.style.minHeight = "100vh";
                /** @type {string} */
                entry.element.style.minWidth = "177.77vh";
              }
            }
          } else {
            if (err) {
  ...
  if (!err) {
    ...
    if (!parse() && (p = document.querySelector("section.menu"))) {
      ...
      if (!p.querySelector(".navbar").classList.contains("collapsed") && 991 < viewport) {
        p.querySelectorAll("ul.navbar-nav li.dropdown").forEach(function(a) {
...
!function() {
  ...
  if (!document.getElementById("top-1")) {
    ...
    /** @type {string} */
    a.innerHTML = '<a href="https://mobiri.se/aa">Free Website Generator</a> Mobirise v5.6.8 <a href="https://mobiri.se/a3">Web Page Designs</a>';
